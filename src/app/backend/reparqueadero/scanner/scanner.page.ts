import { Invitados, RegistroPPrincipalInvi, RegistroPPrincipalResi } from './../../../models/models';
...
export class ScannerPage implements OnInit {
  ...
  async nuevainvien(nuevos: Invitados, placa){
  ...
  async nuevaresien(nueves: Usuarios, placa){
    const dateObj = new Date();
    const mes = dateObj.getUTCMonth() + 1;
    const dia = dateObj.getUTCDate();
    const año = dateObj.getUTCFullYear();
  ...
  async nuevainvisa(nuevos: Invitados, placa){
    const dateObj = new Date();
    const mes = dateObj.getUTCMonth() + 1;
    const dia = dateObj.getUTCDate();
    const año = dateObj.getUTCFullYear();
  ...
  async nuevaresisa(nueves: Usuarios, placa){
    const dateObj = new Date();
    const mes = dateObj.getUTCMonth() + 1;
    const dia = dateObj.getUTCDate();
    const año = dateObj.getUTCFullYear();
  ...
  SalidaInvi(nuevos: Invitados, auto){
    if(auto === null){
      this.interaction.presentToast('Debe ingresar el auto');
    }else{
      this.interaction.presentLoading('Registrado Invitado...');
      const path = 'RegistroPA';
      this.nuevainvisa(nuevos, auto);
  ...
  EntradaResi(nueves: Usuarios, auto){
    if(auto === null){
      this.interaction.presentToast('Debe ingresar el auto');
    }else{
      this.interaction.presentLoading('Agregando Invitado...');
      const path = 'RegistroPA';
      this.nuevaresien(nueves, auto);
  ...
  SalidaResi(nueves: Usuarios, auto){
    if(auto === null){
      this.interaction.presentToast('Debe ingresar el auto');
    }else{
      this.interaction.presentLoading('Registrado Invitado...');
      const path = 'RegistroPA';
      this.nuevaresisa(nueves, auto);
