...
export class PrincipalComponent implements OnInit {
  ...
  Todos(){
    ...
    this.firestoreService.getCollectionUnicMasDouble<Registro>(path, 'dia', diasemi, 'tipo', 'Invitado').subscribe( res => {
      this.semanal = res;
    });
    if(dia>15){
  ...
  BuscarDiarioTipo(tipo){
    ...
    const dia = dateObj.getUTCDate();
    const path = 'RegistroPP';
    this.firestoreService.getCollectionUnicDescTriple<Registro>(path, 'dia', dia, 'numtipo', tipo, 'tipo', 'Invitado').subscribe( res => {
      this.diariotipo = res;
    });
  }
  ...
  BuscarSemanalTipo(tipo){
    ...
    const path = 'RegistroPP';
    this.firestoreService.getCollectionUnicMasTriple<Registro>(path, 'dia', diasem1, 'numtipo', tipo, 'tipo', 'Invitado').subscribe( res => {
  ...
  BuscarQuincenalTipo(tipo){
    ...
    const dia = dateObj.getUTCDate();
    if(dia>15){
      this.firestoreService.getCollectionUnicDescTriple<Registro>(path, 'qui', 2, 'numtipo', tipo, 'tipo', 'Invitado').subscribe( res => {
        this.quincenaltipo = res;
      });
    }else{
      this.firestoreService.getCollectionUnicDescTriple<Registro>(path, 'qui', 1, 'numtipo', tipo, 'tipo', 'Invitado').subscribe( res => {
        this.quincenaltipo = res;
      });
    }
  }
  ...
  BuscarMensualTipo(tipo){
    ...
    const mes = dateObj.getUTCMonth() + 1;
    const path = 'RegistroPP';
    this.firestoreService.getCollectionUnicDescTriple<Registro>(path, 'mes', mes, 'numtipo', tipo, 'tipo', 'Invitado').subscribe( res => {
  ...
  BuscarSemestralTipo(tipo){
    ...
    const mes = dateObj.getUTCMonth() + 1;
    if(mes>6){
      this.firestoreService.getCollectionUnicDescTriple<Registro>(path, 'sem', 2, 'numtipo', tipo, 'tipo', 'Invitado').subscribe( res => {
        this.semestraltipo = res;
      });
    }else{
      this.firestoreService.getCollectionUnicDescTriple<Registro>(path, 'sem', 1, 'numtipo', tipo, 'tipo', 'Invitado').subscribe( res => {
        this.semestraltipo = res;
      });
    }
  }
  ...
  BuscarAnualTipo(tipo){
    ...
    const año = dateObj.getUTCFullYear();
    const path = 'RegistroPP';
    this.firestoreService.getCollectionUnicDescTriple<Registro>(path, 'año', año, 'numtipo', tipo, 'tipo', 'Invitado').subscribe( res => {
      this.anualtipo = res;
    });
  }
