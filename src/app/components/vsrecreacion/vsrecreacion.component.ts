...
@Component({
  selector: 'app-vsrecreacion',
  templateUrl: './vsrecreacion.component.html',
  styleUrls: ['./vsrecreacion.component.scss'],
})
export class VsrecreacionComponent implements OnInit {
  ...
  async apartarCupo(nuevos: Modelo, resi: number, invi: number, nuevo: Usuarios){
    const uid = await this.firebaseauthService.getUid();
    const path = 'MoldeHorarios/Recreacion/cupo';
    const data: Cupo = {
      uid,
  ...
  addCupo(estado: boolean, resi: number, invi: number,){
    ...
    this.firestoreService.updateDoc(path, 'Recreacion', data).then(res =>{
