import { RegistroCS } from './../../models/models';
import { Compromiso, Cupo, Horarios, Modelo, Usuarios } from 'src/app/models/models';
...
@Component({
  selector: 'app-vsgimnasio',
  templateUrl: './vsgimnasio.component.html',
  styleUrls: ['./vsgimnasio.component.scss'],
})
export class VsgimnasioComponent implements OnInit {
  ...
  async apartarCupo(nuevos: Modelo, resi: number, invi: number, nuevo: Usuarios){
    const uid = await this.firebaseauthService.getUid();
    const path = 'MoldeHorarios/Gimnasio/cupo';
    const data: Cupo = {
      uid,
  ...
  addCupo(estado: boolean, resi: number, invi: number,){
    ...
    this.firestoreService.updateDoc(path, 'Gimnasio', data).then(res =>{
